...
= let base = pred.base in
  let pr = pred.pred in
  {
    step = LParse.WellFounded.forall_list_f_weak base;
    fold = LParse.WellFounded.forall_list_f_weak pr;
    prf = (fun aux x ->
      pred.prf x;
      LParse.WellFounded.for_all_fold_left_aux pr (aux && base x) (get_children s x)
    );
  }
