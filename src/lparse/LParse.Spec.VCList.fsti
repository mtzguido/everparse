...
module LParse.Spec.VCList
include LParse.Spec.Combinators // for seq_slice_append_l
include LParse.Spec.Array // for vlarray
...
= if n = 0
  then ()
    ...
    LParse.Math.distributivity_add_left (n - 1) 1 k.parser_kind_low;
...
= if n = 0
  then ()
    ...
    | None -> ()
    | Some b -> LParse.Math.distributivity_add_left (n - 1) 1 b
    end;
