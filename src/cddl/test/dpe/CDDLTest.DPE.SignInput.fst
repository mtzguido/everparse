...
requires rel_vercdl_signinputargs x w
ensures 
  rel_vercdl_bytes x.intkey5 w._x4 **
  Trade.trade (rel_vercdl_bytes x.intkey5 w._x4)
              (rel_vercdl_signinputargs x w)
{ 
  unfold_with_trade
    (`%rel_vercdl_signinputargs) 
    (rel_vercdl_signinputargs x w);
  destruct_rel_fun _ _ _ _ _;
  Trade.trade_compose _ _ (rel_vercdl_signinputargs x w);
  fold_last_relation (`%tstr_any) tstr_any;  
  let rest_5 = fst_pair _ _ _ _ _;
  let tbs = snd_pair _ _ _ _ _;
  with xx yy. assert (rel_vercdl_bytes xx yy);
...
ensures exists* p s.
  ...
  Trade.trade (pts_to tbs #p s)
              (rel_vercdl_signinputargs x w) **
  pure (bytes_of_vercdl_bytes s w._x4)
{
...
let is_tbs_bytes (tbs_bytes:Seq.seq UInt8.t) (w:Seq.seq UInt8.t) =
  exists (wx:spect_vercdl_signinputargs_pretty) (wr:Seq.seq UInt8.t).
          validate_and_parse_postcond_some bundle_signinputargs.b_spec.parser w wx wr /\
          wx._x4 == spect_vercdl_bytes_pretty_right (spect_vercdl_bstr_pretty_right tbs_bytes)
